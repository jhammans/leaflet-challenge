...
function createPlatesLayer(url) {
    let tectonicLayer = L.layerGroup(); // Create a layer group for tectonic plates
   
    d3.json(url).then(function(data) {
      // Add GeoJSON layer to display the tectonic plate boundaries
    L.geoJSON(data, {
        style: function(feature) {
          return {
            color: "#28a745", // Plate boundary color
            weight: 4,
            opacity: 1,
            fillOpacity: 0.
          };
    ...
    return tectonicLayer; // Return the created tectonic layer
  }
...
// Create the layer groups for tectonic plate dataset
let platesLayer = createPlatesLayer(urlPlates);
